<template>
    <MainLayout>
        <div class="flex flex-col p-5">
            <div>
                <button class="p-2 bg-green-700 rounded text-zinc-200 font-semibold" @click="btnCreate"> Add Category
                </button>
            </div>
            ...
            <div class="mt-4">
                <input v-model="search" id="search"
                    class="border-2 outline-none focus:border-red-300 rounded border-red-500 text-center text-sm w-[25%] h-8"
                    placeholder="search" />
            </div>
                    ...
                    <template #item-action="item">
                        <div class="flex flex-row gap-2">
                            <template v-if="!item.is_deleted">
                                <div @click="onUpdate(item)"><button
                                        class=" text-white text-[font-size:8px] w-10 rounded bg-blue-400">Edit</button>
                                </div>
                                <div @click="onDelete(item)"><button
                                        class=" text-white text-[font-size:8px] w-14 rounded bg-red-500">Delete</button>
                                </div>
                            </template>
