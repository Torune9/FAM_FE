                    ...
                    <section class="flex flex-col">
                        <div>
                            <div class="h-14 relative group">
                                ...
                                <input type="text" id="username" name="username" v-model.trim="form.username"
                                    class="bg-transparent border border-slate-400 outline-none rounded-md w-64 h-9 pl-7 pr-2 text-[11px] text-white font-semibold group-hover:border-slate-600"
                                    placeholder="username">
                                <p class="text-[11px] text-center text-red-500 font-bold placeholder-white/70"
                                    v-for="error, index of v$.username.$errors" :key="index">
                                    {{ `*${error.$message}` }}
                                </p>
                            </div>
                            <div class="h-14 relative group">
                            ...
                            <div class="h-14 relative group flex w-64 flex-wrap gap-x-1">
                                ...
                                <input :type="type" id="password" name="password" v-model.trim="form.password"
                                    class="bg-transparent border border-slate-400 outline-none rounded-md w-[218px] h-9 pl-7 pr-2 text-[11px] text-white font-semibold group-hover:border-slate-600"
                                    placeholder="password">
                                <button @click="showHide"
                                    class="inline-block border border-slate-400 hover:text-slate-300 w-8 h-9 rounded-md">
                                    <font-awesome-icon :icon="eye" />
                                </button>
                                <p class="text-[11px] w-full text-center text-red-500 font-bold placeholder-white/70"
                                    v-for="error, index of v$.password.$errors" :key="index">
                                    {{ `*${error.$message}` }}
                                </p>
                            </div>
                        </div>
                        ...
                        <div>
                            <div class="flex justify-center">
                                <button @click="register"
                                    class="h-9 bg-indigo-600 w-full rounded hover:bg-indigo-500 font-semibold transition-all">
                                    Register
                                </button>
                            </div>
                            <br>
