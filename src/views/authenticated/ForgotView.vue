<template>
  <main>
    ...
    <BaseLayoutAuth>
      <template #auth-slot>
        <div
          class="rounded-md flex justify-center items-center flex-wrap h-[400px] w-[400px] max-[450px]:w-[350px] max-[450px]:h-[350px] p-4 shadow-lg bg-slate-800 text-zinc-200 relative">
          ...
          <form @submit.prevent="forgotPw" class="flex flex-wrap gap-5">
            <div>
              ...
              <input v-model="payload.email" type="text" id="email"
                class=" w-60 h-9 outline-none border border-slate-600 rounded-md text-center text-sm bg-transparent"
          ...
          <div class="relative">
            <small
              class="text-white before:absolute before:bottom-0 before:contents[''] before:bg-white before:h-[2px] before:w-[34px] before:mr-3 active:text-slate-400 before:scale-x-0 hover:before:scale-x-100 before:transition-transform active:scale-x-100 font-semibold"><router-link
                to="/login">Back</router-link></small>
          </div>
        </div>
      </template>
    </BaseLayoutAuth>
